...
- [x] Comprehensive docstrings for all DSL forms and MCP tools — Every `PipelineBuilder` method in `dsl.py` now has a complete docstring with signature, description, Args section, Returns, Example, and Notes. Key forms upgraded: `show()` (all 15 display_props documented), `stream_tracer()`, `glyph()`, `extract_grid()`, `tube()`, `gradient()`, `mask_points()`, `warp_vector()`, `warp_scalar()`, `cell_to_point()`, `point_to_cell()`, `outline()`, `elevation()`, `surface()`, `smooth()`, `clip()`, `clip_box()`, `clip_sphere()`, `slice()`, `probe()`, `resample_to_image()`, `seeds_near()`, `line_probe()`, `raw_source()`, `make_vector()`, `compute_velocity()`, `compute_magnitude()`, `curl()`, `compute_vorticity()`, `compute_gradient_magnitude()`, `camera()`, `background()`, `scene_preset()`, `title()`. MCP tools `set_pipeline()`, `list_capabilities()`, `get_dsl_reference()` also improved. All `_EXAMPLES` in `get_dsl_reference()` expanded with concrete, copy-pasteable examples. Docs regenerated: `docs/dsl-reference.md` (67,073 chars) and `docs/mcp-reference.md`.
