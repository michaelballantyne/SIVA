...
- [x] Consolidate DSL discovery into a single entry point: Renamed get_examples
and merged list_capabilities into get_dsl_overview() (workflow patterns on
top, full form index with one-line descriptions below, VTK classes, colormaps).
Added "see also" cross-references in get_dsl_reference for easily confused forms
(extract_region/slice/clip, contour/isosurface aliases, make_vector/compute_velocity
aliases, compute_magnitude vs compute_vorticity distinction). Updated MCP server
WORKFLOW to call get_dsl_overview() as step 1.
...
- [ ] Fix `title()` actors not cleared on `set_pipeline` rebuild — When `set_pipeline` is called multiple times on the same view, previous `title()` text actors persist and overlap with new ones. The pipeline rebuild clears 3D actors but not 2D text overlays. `title()` actors should be tracked separately and cleared at the start of each `set_pipeline` execution, similar to how `_actors` are cleared. Related to the "Separate 2D overlay actors" item below.
