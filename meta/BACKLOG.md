...
- [ ] Spatial-region statistics — `get_statistics` operates on the whole dataset. Add a `bounds` parameter (physical coordinates) or accept a pipeline node (post-clip/threshold) so statistics can be computed on subregions. In the bonsai session this would have shortened density exploration from 20+ rounds to a few. Convergent evidence: mentioned in bonsai feedback and implied by wildfire sessions.
