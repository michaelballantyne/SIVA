...
- [ ] File-watching hot reload with status file — Watch pipeline files for changes and auto-rebuild on save. Write build output (success summary or error) to a status file next to the pipeline file with a matching name (e.g. `view-main.py` -> `view-main.status.txt`) so the human can see it in a split view. Must work with multiple views, each getting its own status file. Replaces `set_pipeline` as the primary build trigger — Claude just writes the file and reads the status file afterward; the human just saves and checks status. `screenshot()` should also include the latest build status in its text result. Update server instructions to guide Claude to read `.vislang/status.txt` after writing a pipeline file to check for errors before taking a screenshot — avoids wasting an image on a broken build. Supersedes the "inline code" and "read pipeline before set_pipeline" items.
- [ ] Fix multi-view crash in interactive mode — `new_view()` causes "Connection closed" server crash when a VTK display window is open (observed in bonsai session). Needs investigation into whether multiple VTK render windows are supported or if views should share a single window.
- [ ] Remove auto-screenshots from state-changing tools to fix context bloat — Currently every mutation tool (set_pipeline, set_camera, set_colormap, toggle_visibility, etc.) auto-returns a base64 screenshot via `_with_screenshot()`. In long sessions this accumulates tens of MB of image data in Claude's context, eventually hitting the 20MB API request limit. Fix: (1) Remove `_with_screenshot()` from all mutation tools so they return text only. (2) Add `resolution` parameter to `screenshot()` with "low" (e.g. 640x480) and "high" (1920x1080) options, defaulting to low. (3) Update server instructions to guide Claude to call screenshot() in the same turn as a state-changing tool when it wants to see the result (parallel tool calls), start with low-res, and use high-res only when detail is needed. (4) Batch Write + set_pipeline guidance. (5) `camera_orbit` and `render_chart` also need adjustment — return file paths or reduce resolution.
- [ ] Remove mutation tools in favor of pipeline file edits — With hot reload, `set_colormap`, `set_opacity`, `toggle_visibility`, `set_background`, `annotate`, `clear_annotations` become unnecessary — Claude edits the `show()` / `background()` lines in the pipeline file and the server picks up the change. This eliminates the divergence where mutation tools change VTK state without updating the pipeline file (making the file lie about what's rendered). `set_camera` is the exception — camera is interactive (human rotates with mouse) and shouldn't live in the file unless explicitly frozen. Depends on hot reload being implemented first.
...
- [ ] Consolidate reference tools — Rename `get_examples` to `get_dsl_overview` (intro + concepts + example pipelines, the "start here" tool). Merge `list_capabilities` into `get_dsl_reference` so no-arg `get_dsl_reference()` lists all forms with one-line descriptions, and `get_dsl_reference(form)` gives full docs on one form. Result: two reference tools with clear roles instead of three with overlapping purpose.
- [ ] Improve error messages to not leak Pydantic internals — Validation errors like `"1 validation error for set_pipelineOutput\nresult\nInput should be a valid string"` leak implementation details instead of surfacing the actionable pipeline-level message. Error responses should surface the pipeline error, not the serialization failure. The return-type annotation fixes from the auto-screenshot removal will help, but the error path deserves an explicit pass.
...
- [ ] Context-aware `get_examples` — Filter examples by loaded data type and substitute real field names from the active pipeline. The previous attempt was reverted because blind field substitution was misleading. A better approach: show examples tagged by data type (structured grid, image data, polydata) and only substitute field names when the semantics are clear (e.g. the first scalar field, the first vector field). The bonsai session confirmed Claude doesn't always call `get_statistics` before choosing threshold values — richer examples would help.
...
- [ ] Interactive parameter adjustment / parameterized pipelines — The bonsai user wanted sliders for density limits. Full widgets are out of scope, but a lighter approach: support pipeline templates with named parameters (e.g. `threshold_value = {{density_threshold}}`), so Claude can call `set_pipeline_param(density_threshold=120)` without rewriting the whole file. Reduces the edit-set_pipeline-screenshot cycle for exploration.
...
- [ ] MCP server reconnection / tool re-registration — The bonsai session hit a mid-session crash where reconnection failed because tools weren't re-registered. Investigate whether the MCP SDK supports automatic tool re-registration on reconnect, or if there's a server-side fix.
