...
def get_statistics(node: str, field: str) -> str:
    ...
    Use this to understand value ranges before setting thresholds, isosurface values, etc.
    """
    data = _get_data(node)
...
def get_spatial_extent(node: str, field: str, min_value: float, max_value: float) -> str:
    ...
    Useful for positioning seed points for streamlines or focusing cameras.
    """
    data = _get_data(node)
