...
- [ ] Consolidate duplicated constants — `_scalar_type_map` is defined independently in `dsl.py` and `filters.py`; `_component_name_map` (`{"x":0,"y":1,"z":2}`) appears in four places (`filters.py:72`, `filters.py:959`, `dsl.py:161`, `server.py:403`). Move each to a single module-level constant and import it everywhere. Also unify the reader-extension maps (`EXT_TO_READER` in `filters.py` vs the local `readers` dict in `server.py:quick_start`, which omits `.vtr`). (code-quality-reflection 3.5, 3.6, 3.14)
- [ ] Extract node-lookup boilerplate into a helper — 16 MCP tool functions repeat the same 4-line `_get_data` / error-return pattern. A `@requires_data` decorator or `_get_data_or_error()` helper would eliminate ~64 lines of duplication and ensure consistent error messages. (code-quality-reflection 3.4)
