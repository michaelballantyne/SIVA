...
- [x] Generalize `compute_velocity`/`compute_vorticity` into `make_vector` + `curl` — `make_vector(input, components, result)` is the general "assemble scalars into a vector" primitive; `curl(vector_field, result, vector)` is the general curl operator. `compute_velocity` and `compute_vorticity` are now thin wrappers. Both added to DSL (`dsl.py`), the DSL namespace (`interpret()`), and as MCP tools in `server.py`. 26 tests in `tests/test_make_vector_curl.py`.
...
- [x] Integration tests pass under pytest — downloaded wildfire dataset and ran the full integration test suite. Fixed two blocking issues: (1) conftest.py now starts Xvfb automatically in headless environments so VTK can get a GLX context; (2) renamed the `test` decorator factory to `_register` so pytest doesn't collect it as a test fixture. All 46 integration tests pass (315 total in the suite).
