...
Returns cache stats, pipeline variables, captured print output, and any errors
from the most recent execution (including watcher-triggered reloads). Use this
to verify that your latest file edits were picked up and executed successfully.
...
A formatted status report containing:
- Whether the file watcher is running
- Cache stats: hits, misses, evictions
- Pipeline variable names
- Any captured `print()` output from the last execution
- Any error from the last execution

### Example

```
pipeline_status("fire.py")
```

```
Pipeline status for 'fire':
  Watcher running: True
  Cache stats: hits=8, misses=3, evictions=0
...
Pipeline status for 'fire':
  Watcher running: True
  Cache stats: hits=2, misses=0, evictions=0
  Pipeline variables: mesh, hot
  Last error:
  NameError: name 'threshold' is not defined. Did you mean: 'mesh.threshold(...)'?
```
