...
from tracked_execution import DAG, tracked_read, vtk_escape_multi
from utils import cleanup, create_test_dataset


# ---------------------------------------------------------------------------
# Multi-input function — defined at module scope.
# vtk_escape_multi calls func(*real_inputs) — positional args, not a list.
# ---------------------------------------------------------------------------
...
def run_pipeline(data_path, dag, hot_threshold, cold_threshold, colormap):
    """Read → threshold(hot) + threshold(cold) → vtk_escape_multi(merge) → show."""
    dag.begin_run()

    mesh = tracked_read(data_path, dag)

    # Isolate hot region (above hot_threshold)
    hot = mesh.threshold(value=hot_threshold, scalars="Temperature")

    # Isolate cold region (below cold_threshold, using invert=True)
    cold = mesh.threshold(value=cold_threshold, scalars="Temperature", invert=True)

    # Merge both regions in a single cached escape
    combined = vtk_escape_multi([hot, cold], combine_regions)

    real_hot = object.__getattribute__(hot, "_real")
    real_cold = object.__getattribute__(cold, "_real")
    real_combined = object.__getattribute__(combined, "_real")

    dag.end_run()

    return dag.stats(), real_hot.n_points, real_cold.n_points, real_combined.n_points
...
def main():
    ...
    runs = [
        ...
        (700.0, 300.0, "coolwarm",
         "Run 1: cold cache — thresholds and vtk_escape_multi all miss"),
        (700.0, 300.0, "coolwarm",
         "Run 2: same pipeline — all hits"),
        (700.0, 300.0, "plasma",
         "Run 3: colormap only — all mesh ops (incl. vtk_escape_multi) hit"),
        (750.0, 300.0, "coolwarm",
         "Run 4: hot threshold raised — hot + vtk_escape_multi miss; cold hits"),
        (750.0, 250.0, "coolwarm",
         "Run 5: cold threshold lowered — cold + vtk_escape_multi miss; hot hits"),
        (750.0, 250.0, "coolwarm",
    ...
    for hot_threshold, cold_threshold, colormap, description in runs:
        ...
        stats, n_hot, n_cold, n_combined = run_pipeline(
            data_path, dag, hot_threshold, cold_threshold, colormap
        )
        elapsed = time.perf_counter() - t0
        results.append((description, stats, elapsed, n_hot, n_cold, n_combined))
    ...
    print("-" * 60)
    for i, (description, stats, elapsed, n_hot, n_cold, n_combined) in enumerate(results, 1):
        print(
            f"{i:<6}  {stats['hits']:>5}  {stats['misses']:>7}"
            f"  {stats['evictions']:>10}  {elapsed:>8.4f}"
        )
        print(f"       {description}")
        print(f"       -> hot={n_hot} pts  cold={n_cold} pts  combined={n_combined} pts")
    ...
    # Run 3: colormap only — vtk_escape_multi is a hit
    s3 = results[2][1]
    ...
    s4 = results[3][1]
    # read() + cold region = at least 2 hits; hot + vtk_escape_multi = at least 2 misses
    assert s4["hits"] >= 2, (
